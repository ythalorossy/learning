...
sudo docker tag bc5c421ecd6c ythalorossy/hello-app:ps


#Now push that locally tagged image into our private registry at docker hub
#You'll be using your own repository, so update that information here. 
sudo docker push ythalorossy/hello-app:ps
...
kubectl create secret generic private-reg-cred \
    --from-file=.dockerconfigjson=/home/yross/.docker/config.json \
...
#you'll be using your own repository, so update that information here.
ssh yross@c1-node1 'sudo docker rmi ythalorossy/hello-app:ps'
ssh yross@c1-node2 'sudo docker rmi ythalorossy/hello-app:ps'
ssh yross@c1-node3 'sudo docker rmi ythalorossy/hello-app:ps'
